...
点击列表中任一地区的时间，直接键入完整日期时间，所有地区立即联动。页面只保留世界时钟、四个悬浮图标与必要反馈，不设置独立输入区或解释面板。
...
- 右下角恢复到现在、复制所示时间；复制格式统一为 `PDT | 2026-08-04 21:13`。
...
在 `src/data/timeZones.ts` 的 `TIME_ZONES` 中加入中文名、英文名、IANA 时区和缩写映射。列表、UTC 偏移、编辑、地区选择和复制会共用同一配置。
